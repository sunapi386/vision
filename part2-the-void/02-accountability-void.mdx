...
There is a fourth missing piece that cuts across the other three: knowing how much weight to put on an answer.

A professional can say, "The Southeast Asian market data is thin; check those numbers before the board meeting." An AI summary may present a sourced fact and a guessed statistic in the same authoritative voice. A system that is certain and wrong can be more dangerous than one that admits uncertainty: it steers attention away from the mistake.

The problem is measurable. In Liu et al.'s [ICLR 2025 study of guard-model calibration](https://proceedings.iclr.cc/paper_files/paper/2025/file/a99f732df9b668284b449da0214a3286-Paper-Conference.pdf), even the best average prompt-classification ECE among the evaluated models was 14.4%. That is an aggregate calibration error in that evaluation, not an error bound for each answer.

Calibration asks whether comparable predictions scored at 80% turn out correct about 80% of the time under the evaluation conditions. Several models agreeing is evidence to investigate, not a calibrated probability by itself. Shared blind spots, changing inputs, and mistaken labels all matter. Human reviewers need measured performance and feedback too; being human does not make their judgments the ground truth.

Confidence can help the accountant decide where to begin among 10,000 overnight invoices. The amount at stake, reversibility, and possible cumulative loss also belong in that decision. Sampling apparently easy cases helps discover errors that the score failed to flag.

**When the reviewer has neither time to investigate nor authority to intervene, human-in-the-loop governance is theater.** A review queue needs evidence the person can inspect, room to disagree, and a way to stop or correct the work. A better score cannot supply those powers.

The tools for evaluation and calibration exist. The work is to connect them to everyday operations: record the task and model version, check predictions against outcomes, retain disagreements and corrections, and re-evaluate when the setting changes. The routing signal earns trust through those measurements.
...
Attorneys have been sanctioned in multiple jurisdictions for submitting AI-generated briefs with fabricated case references. These aren't isolated incidents; they're the predictable result of deploying systems that generate authoritative-looking citations without any provenance mechanism. Without citation tracking infrastructure, every AI-assisted legal filing is a potential time bomb.

PII leaks across organizational boundaries. When customer data passes through AI agent workflows that span multiple services, there is no mechanism to ensure that PII is stripped, masked, or governed at each boundary. Samsung banned employee use of ChatGPT after discovering that employees had uploaded proprietary source code and internal meeting notes. The data reached OpenAI's servers, where it was processed, potentially cached, and possibly incorporated into training data that would be served to other customers.

Costs go unattributed. Organizations routinely discover that their AI spending is 3-5x what they budgeted, because the per-token costs accumulate invisibly across dozens of workflows and hundreds of daily operations. Without per-workflow cost attribution, the only signal is the monthly invoice, and by then, the spending has already occurred.

And compliance fails by default. Organizations subject to HIPAA, GDPR, SOX, or other regulatory frameworks are deploying AI systems that cannot produce the records these frameworks require. They're betting that regulators won't ask for AI-specific audit trails yet. When the regulators do ask (and the seven forces described in the previous chapter guarantee they will), the organizations will discover that the records don't exist and can't be reconstructed.

The accountability void is a structural gap in the infrastructure, and it will widen as AI adoption accelerates; the market will not resolve it on its own. More agents, more cross-org workflows, more data flowing across more boundaries, with the same absence of tracking, attribution, and governance at every step.
