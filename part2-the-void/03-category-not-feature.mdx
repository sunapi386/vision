...
The distinction between the control plane approach and the accountability layer approach is a structural constraint that determines what's possible, not a matter of engineering preference.
...
The agent economy needs its equivalent. Not a narrative log of what happened, which is the current approach of API call logs and billing dashboards. A structural format for accountability records that composes recursively, travels with the data, enforces constraints at every boundary, and produces artifacts that any party can audit independently.

This does not bolt onto an existing agent framework, and it is not a dashboard beside the workflow or a monitoring service watching from outside. It is a category of infrastructure, as fundamental to the agent economy as double-entry bookkeeping is to the financial economy.
