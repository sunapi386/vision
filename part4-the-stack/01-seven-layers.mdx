...
The numbers are worse than the intuition suggests. In entity extraction tasks, 86% of entities receive maximum confidence scores from single models, yet only 40% of those entities are actually correct. The model is not uncertain and wrong -- it is certain and wrong, which is far more dangerous. Published guard models designed specifically for safety -- Llama Guard, Shield Guard, Wild Guard -- exhibit Expected Calibration Error between 14% and 28%. These are not general-purpose models being misused. They are purpose-built safety tools that cannot reliably distinguish their own accuracy from their own confidence.
