...
**Data governance enforcement.** The client that controls the data must authorize access on its own terms. An agreement between the consulting and research firms cannot grant rights to the client's data. At this boundary, the client verifies identity, purpose, permissions, and any required consent. Conditions such as restricted retention need enforceable controls and a clearly stated threat model.
...
- **Verifiable.** The provenance chain, cost tree, and governance records are structured data that can be independently audited. They are cryptographically signed records produced by the accountability layer as a natural byproduct of execution, not self-reported claims.
...
As context flows downward, each delegation may narrow the authority it receives, but cannot widen it. An increase requires a separate grant from an authorized party. Enforcing this requires checks outside the acting agent, shared accounting for concurrent budget commitments, expiry, and revocation that reaches outstanding work. A written restriction in a prompt is not sufficient.
...
**The trust argument.** An open protocol permits independent implementations and verification. It does not eliminate trust in operators, software, or key management. Organizations should control their records and be able to export workflows, policies, and permitted calibration evidence in usable formats. Conformance tests and migration exercises are needed to make switching vendors practical.

**The audit argument.** Open specifications let auditors examine the meaning of records without depending entirely on one vendor's explanation. Products can be audited too. The protocol's advantage is that multiple implementations can be tested against a shared contract, with their limitations made explicit.

**The durability argument.** Companies can be acquired, shut down, or change direction. An open specification and independent implementations give records a better chance of remaining useful through those changes. Durability also needs versioning, migration tools, long-term verification, and institutions willing to maintain the standard.
...
Interoperability must not silently downgrade a required protection. Ordinary communication can continue when accountability is optional and both parties accept its absence. If the task requires cost limits, provenance, or governance guarantees that a peer cannot provide, the system must refuse that handoff or request an explicit change from an authorized party.
