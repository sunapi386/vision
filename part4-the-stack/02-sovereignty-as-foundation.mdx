...
The preceding chapter organizes the responsibilities of accountable agent work. Deployment adds a practical question: who controls the machines, keys, software updates, and policies through which those responsibilities are exercised?

Sovereignty is the ability to exercise meaningful control over data, compute, policies, and dependencies. Cost records and provenance remain useful in many deployment models. Ownership can strengthen the organization's ability to verify and act on them, but physical location alone does not establish control.
...
Cloud and owned deployments can each support or fail governance requirements. The relevant questions concern authorized processing, access, evidence, location, and contractual responsibilities. Local infrastructure can reduce some dependencies; cloud services can also offer controls that need to be evaluated on their merits.
...
An accountability layer that records costs and provenance is valuable. The same layer running on infrastructure the organization controls, with cryptographic guarantees that the platform operator cannot access the data, is something else entirely: a difference of kind, not degree.

A governance layer should check classifications and permissions at each boundary, including outputs and operational metadata. Owning the surrounding infrastructure can make those checks easier to inspect, but it does not remove errors in classification or administration. The guarantee must describe the protected paths and assumptions.
...
The architecture should support local and disconnected deployments from the outset, alongside governed use of external services. Neither design can be reduced to a checkbox. Each needs explicit assumptions, appropriate controls, and a tested route to recovery or migration.